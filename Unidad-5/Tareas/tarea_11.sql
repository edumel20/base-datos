...
select codigo_pedido, codigo_cliente, fecha esperada, fecha_entrega from pedido WHERE fecha_entrega <= date(fecha_esperada, '-2 days');
...
| 116           | 2008-06-28   | 2008-08-01     | 2008-08-01    | Entregado |                                                           | 38             |
...
select forma_pago from pago GROUP BY forma_pago ORDER BY forma_pago;
...
select c.nombre_cliente, em.nombre, em.apellido1, em.apellido2 from cliente c, empleado em WHERE em.codigo_empleado = c.codigo_empleado_rep_ventas and em.puesto = 'Representante Ventas' GROUP BY cliente;
...
select c.nombre_cliente from cliente c JOIN (select DISTINCT codigo_cliente from pago) AS sin_pagar ON c.codigo_cliente = sin_pagar.codigo_cliente;
...
select c.nombre_cliente from cliente c JOIN (select DISTINCT codigo_cliente from pedido) AS sin_pedido ON c.codigo_cliente = sin_pedido.codigo_cliente;
...
select c.nombre_cliente from clientes c, pedido pe JOIN pago pa ON c.codigo_cliente = pe.codigo_cliente and c.codigo_cliente = pa.codigo_cliente WHERE c.codigo_cliente NOT IN (select codigo_cliente from pedido) and c.cliente_codigo NOT IN (select codigo_cliente from pago);
--Devuelve un listado que muestre solamente los empleados que no tienen una oficina asociada.
/**
Sale una sentencia vacía debido a que todos los empleados tienen una oficina asociada.(se revisó en las tablas)
**/
--Devuelve un listado que muestre solamente los empleados que no tienen un cliente asociado.
select em.nombre from empleado em LEFT JOIN cliente c ON em.codigo_empleado = c.codigo_empleado_rep_ventas WHERE c.codigo_empleado_rep_ventas is null;
...
select em.nombre from empleado em JOIN oficina ofi ON em.codigo_oficina = ofi.codigo_oficina LEFT JOIN cliente c on em.codigo_empleado = c.codigo_empleado_rep_ventas WHERE c.codigo_empleado_rep_ventas is null;
...
select em.* from empleado e LEFT JOIN cliente c ON em.codigo_empleado=c.codigo_empleado_rep_ventas LEFT JOIN oficina ofi ON e.codigo_oficina=o.codigo_oficina WHERE c.codigo_empleado_rep_ventas is null and ofi.codigo_oficina is null;
/**
Devuelve sentencia vacía
**/
--Devuelve un listado de los productos que nunca han aparecido en un pedido.
select pro.* from detalle_pedido dp RIGHT JOIN producto pro ON dp.codigo_producto=pro.codigo_producto WHERE dp.codigo_producto is null;
...
select c.* from cliente c JOIN pedido p ON c.codigo_cliente=p.codigo_cliente LEFT JOIN pago pa ON c.codigo_cliente = pa.codigo_cliente WHERE pa.codigo_cliente is null GROUP BY c.codigo_cliente;
...
select e.*, (j.nombre) AS nombre_jefe from empleado e LEFT JOIN cliente c ON e.codigo_empleado = c.codigo_empleado_rep_ventas LEFT JOIN empleado j ON e.codigo_jefe = j.codigo_empleado WHERE c.codigo_empleado_rep_ventas is null;
...
select COUNT(codigo_empleado) AS numero_empleados from empleado;
...
select COUNT(codigo_cliente) AS numero_clientes, pais from cliente GROUP BY pais;

/**
+-----------------+----------------+
| numero_clientes |      pais      |
+-----------------+----------------+
| 2               | Australia      |
| 2               | France         |
| 27              | Spain          |
...
select AVG(total) AS pago_medio from pago WHERE fecha_pago regexp '2009';
...
--¿Cuántos pedidos hay en cada estado? Ordena el resultado de forma descendente por el número de pedidos.
 select estado, COUNT(codigo_pedido) as numero_pedidos from pedido group by estado;
/**
+-----------+----------------+
...
/**
+-------------------+---------------------+
| producto_mas_caro | producto_mas_barato |
+-------------------+---------------------+
| 462               | 1                   |
+-------------------+---------------------+
**/
...
select COUNT(codigo_cliente) AS clientes_Madrid from cliente WHERE ciudad regexp 'Madrid';
...
select COUNT(codigo_cliente) AS clientes_Madrid from cliente WHERE ciudad regexp 'M';
...
select e.nombre, COUNT(c.codigo_cliente) AS Cliente_Representante from cliente c, empleado e WHERE e.codigo_empleado = c.codigo_empleado_rep_ventas GROUP BY c.codigo_empleado_rep_ventas;
...
select COUNT(codigo_cliente) clientes_sin_representantes from cliente WHERE codigo_empleado_rep_ventas is null;
